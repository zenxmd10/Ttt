...
app.get('/', (req, res) => {
    res.send(`
        <body style="font-family:sans-serif;text-align:center;padding:50px;background:#0f172a;color:white;">
            <div style="background:#1e293b;padding:30px;border-radius:15px;display:inline-block;box-shadow:0 10px 25px rgba(0,0,0,0.3);">
                <h2 style="color:#22c55e;">ZENX-V1 SESSION GENERATOR</h2>
            ...
            <script>
                async function getCode() {
                    ...
                    if(!n) return alert('Enter Number!');
                    document.getElementById('btn').disabled = true;
                    document.getElementById('st').innerText = 'Requesting... Check your WhatsApp notification';
                    const response = await fetch('/getcode?number=' + n);
                    const data = await response.json();
                    if(data.code) {
                        document.getElementById('code').innerText = data.code;
                        document.getElementById('st').innerText = 'Link this code in WhatsApp Settings > Linked Devices';
                    } else {
...
app.get('/getcode', async (req, res) => {
    ...
    const sock = makeWASocket({
        ...
        // Knight Bot-ലെ അതേ ബ്രൗസർ സെറ്റിംഗ്സ്:
